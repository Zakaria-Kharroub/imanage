    ...
    <table class="w-full whitespace-no-wrap">
      <thead>
        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
          ...
          <th><a href="{{route('createRent')}}"><button class="add"> Add a Bill</button></a></th>
        </tr>
      </thead>
        ...
        @if($rent->count()>0)
            @foreach($rent as $rs )
                ...
                <td class="px-4 py-3">
                   ...
                   <a href="{{route('editRent', ['id' => $rs->id])}}"> <button class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Edit">
                        <svg class="w-5 h-5" aria-hidden="true"  fill="currentColor" viewBox="0 0 20 20">
                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                        </svg>
                    </button></a>
                    <form action="{{ route('deleteRent', ['id' => $rs->id]) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <div class="flex items-center space-x-4 text-sm">
